...
public class HU037C3AñadirLinks {
    ...
    public static void main(String[] args) {
        ...
        try (Scanner sc = new Scanner(System.in)) {
            while (true) {
                System.out.println("\n=== Menú de eventos ===");
                System.out.println("1. Buscar evento por título");
                ...
                switch (opcion) {
                    case "1" -> {
                        System.out.print("Escribe el título del evento a buscar: ");
                        String titulo = sc.nextLine().trim().toLowerCase();
                        List<Evento> encontrados = filtrarPorTitulo(eventos, titulo);

                        if (encontrados.isEmpty()) {
                            System.out.println("No se encontraron eventos con el título \"" + titulo + "\".");
                        } else {
